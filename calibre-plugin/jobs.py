    ...
    while True:
        ...
        if not job._processed:
            ...
            try:
                ...
                logger.error("Finished job have no result, faking it")
                fakejobresult = []
                for x in job._site_list:
                    fakejob = {'listorder': x['listorder'] ,'good': False, 'status': _('Error'), 'added': False,
                         'url': x['url'], 'title': x['title'], 'author': x['author'], 'comment': 'Synthetic failure generated by FFF, check Calibre log'}
                    fakejobresult.append(fakejob)
                book_list.extend(fakejobresult)
                pass
            job._processed = True
            jobs_running -= 1
