    ...
    def extractChapterUrlsAndMetadata(self):
        ...
        if authdiv:
            ...
            for a in authdiv.find_all('a'):
                ...
                self.story.addToList('authorUrl', urllib.parse.urljoin(self.url, a['href']))
        ...
        if imgmeta:
            self.coverurl = self.setCoverImage(self.url, urllib.parse.urljoin(self.url, imgmeta['content']))[1]
        ...
        data = [_f for _f in (x.strip() for x in stripHTML(metadiv).split('\xb7')) if _f]
        ...
        for a in soup.find('div', id='rselect')('a'):
            if 'javascript' not in a['href']:
                self.chapterUrls.append((a.get_text(), urllib.parse.urljoin(self.url, a['href'])))
