...
def getClass():
    return XF2testSpaceBattlesComAdapter

class XF2testSpaceBattlesComAdapter(BaseXenForo2ForumAdapter):

    def __init__(self, config, url):
        BaseXenForo2ForumAdapter.__init__(self, config, url)

        # Each adapter needs to have a unique site abbreviation.
        self.story.setMetadata('siteabbrev','sbxf2')

    @staticmethod # must be @staticmethod, don't remove it.
    def getSiteDomain():
        # The site domain.  Does have www here, if it uses it.
        return 'xf2test.spacebattles.com'
    ...
    def getSiteURLPattern(self):
        ...
        return super(XF2testSpaceBattlesComAdapter, self).getSiteURLPattern().replace(re.escape("forums."),r"(forums?\.)?")
