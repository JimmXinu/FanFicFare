...
from tests.conftest import inkittcom_story_html_return, inkittcom_story_api_return, inkittcom_login_api_return, inkittcom_html_chapter_return, inkittcom_html_patreon_return, inkittcom_html_login_patreon_return
...
SPECIFIC_TEST_DATA = {
    ...
    'intro': make_soup("""<p class="story-summary">Parker was the only girl in her family. Her mother was convinced she was a boy so had decided her name while she was in the womb. Her mother and her three brothers all called her Parker even before she was born so the name stuck. 
  ...
  Parker is hurt by her family. Her brothers friend, who had seen everything that had been ha</p>""").p,
    'expected_chapters': {
...
class TestExtractChapterUrlsAndMetadata(GenericAdapterTestExtractChapterUrlsAndMetadata):
    ...
    def setup_env(self):
        with patch(f'fanficfare.adapters.{self.path_adapter}.setDescription') as mock_setDescription, \
            ...
            self.mockpost_request.return_value = inkittcom_login_api_return
    ...
    def test_raises_fic_patreon_exclusive(self, mockget_request):
        # Given
        mockget_request.side_effect = [inkittcom_html_patreon_return, inkittcom_html_login_patreon_return]
        ...
        # Then
        assert str(exc_info.value) == "Book is only available for patreons of this author."
